"""박람회 준비 데스크 리서치 (v7.0) — 국가별 주요 사이트 우선 검색 + 페르소나 부스 컨셉 기획
...
VERSION = "v7.0"               # 질문·프롬프트·규칙을 바꾸면 올려서 리포트 캐시 무효화
MODEL = "gpt-4o-mini"
...
def plan_booth_concept(client, product_name, country, exhibition_name, cards, quote_index, profile):
    """B안: 기획자 초안 → 바이어 검토 → 기획자 수정. A안(BOOTH_REVIEW=False)은 초안만.
    뒤 단계가 실패하면 앞 단계 결과를 쓴다."""
    facts = _booth_facts(product_name, country, exhibition_name, cards, quote_index, profile)
    try:
        draft = _ask_json(client, f"""아래 조사 결과를 바탕으로 {country} 박람회 부스 컨셉 기획안을 작성하세요.
{GUARD}
...
JSON 형식:
{BOOTH_SCHEMA}""", temperature=0.6, model=BOOTH_MODEL, system=PLANNER_PERSONA)
    except Exception as e:
...
- overall: 한 줄 총평 (한국어)
- comments: 항목별 지적 3~6개. section은 main_visual/slogan/signature_pairing/demonstration/packaging/merchandising/overall 중 하나,
  issue(문제), suggestion(바이어가 원하는 개선 방향). 한국어.
JSON: {{"overall": "", "comments": [{{"section": "", "issue": "", "suggestion": ""}}]}}""",
                               temperature=0.3, model=BOOTH_MODEL, system=REVIEWER_PERSONA)
            revised = _ask_json(client, f"""당신이 쓴 부스 기획 초안에 바이어 검토 의견이 왔습니다. 의견을 반영해 최종안을 쓰세요.
...
{BOOTH_SCHEMA[:-1]},
  "applied": [""]
}}""", temperature=0.5, model=BOOTH_MODEL, system=PLANNER_PERSONA)
            if isinstance(revised, dict) and revised.get("summary"):
                final, stage = revised, "revised"
        except Exception as e:
            print(f"부스 컨셉 검토·수정 실패 (초안 사용): {e}")
...
def run_research(product_name, country, *, exhibition_name=None, exhibition_website=None,
    ...
    key = _hash({"v": VERSION, "m": MODEL, "bm": BOOTH_MODEL, "br": BOOTH_REVIEW, "p": product_name, "c": country,
