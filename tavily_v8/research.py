"""박람회 준비 데스크 리서치 (v8.0) — 주요 사이트 우선 검색 + 스킬 문서 기반 부스 컨셉 기획
...
VERSION = "v8.0"               # 질문·프롬프트·규칙을 바꾸면 올려서 리포트 캐시 무효화
MODEL = "gpt-4o-mini"
...
BRIEF_SCHEMA = """{
  ...
  "big_idea": {"line_en": "", "line_ko": "", "why": "왜 이 약속인가 (근거 요약)", "basis": ["기업:강점", "E3"]},
...
BOOTH_SCHEMA = """{
  ...
  "main_visual": {"concept_en": "", "concept_ko": "", "key_message": "",
                  "bullets": [{"text": "", "basis": ["E12"]}]},
  "slogan": {"main_en": "", "main_ko": "", "sub_en": "", "sub_ko": "",
             "bullets": [{"text": "슬로건 선정 이유", "basis": ["기업:강점"]}]},
  "signature_pairing": {"item": "", "intent": "", "bullets": [{"text": "", "basis": ["E4"]}]},
...
- 각 불릿의 basis에 근거를 적으세요: 조사 발췌 id("E12"), 기업 정보("기업:강점"), 근거 없는 아이디어는 "기획".
  조사 발췌를 활용할 수 있는 곳은 적극적으로 활용하세요.
...
def plan_booth_concept(client, product_name, country, exhibition_name, cards, quote_index, profile):
    """스킬 문서 기반 부스 컨셉 기획.
...
믿을 이유 2~4개. 근거는 basis에 발췌 id("E12"), 기업 정보("기업:강점"), 근거 없으면 "기획". 설명은 한국어.
JSON 형식:
    ...
    with _locks[key]:
        ...
        try:
            ...
            try:
                ...
                booth = plan_booth_concept(client, product_name, country, exhibition_name, cards, quote_index, profile)
            except Exception:
