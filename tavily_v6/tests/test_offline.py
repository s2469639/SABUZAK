"""네트워크 없이 v6(주요 사이트 우선 검색 + 충분성 판정) 로직을 검증하는 오프라인 테스트.
...
def fake_ask_json(client, prompt, temperature=0.0):
    p = prompt
    ...
    if "부스를 기획하는 마케터" in p:
        return {"key_message": "메시지", "reasons": [], "ideas": []}
...
class V6PipelineTest(unittest.TestCase):
    ...
    def test_flask_render(self):
        ...
        with mock.patch.object(research, "DEFAULT_DB_PATH", self.db):
            html = web.app.test_client().post("/", data={"product_name": "약과", "country": "United States"}).get_data(as_text=True)
        for text in ["v6 · 주요 사이트 우선", "검색 범위", "주요 사이트 자료만으로 충분", "일반 웹까지 검색",
