# v14 — 페어메이트 트렌드 조사 탭 & 부스 컨셉 기획

v13을 바탕으로 한 단독 폴더입니다. v12를 바탕으로 **트렌드 조사(1~3번)**와 **부스 컨셉(4번)**을 서로 다른 작업·화면으로 나눈 단독 폴더입니다.
...
## v14 변경
- 트렌드 수치(전년 대비 %, 급상승, 급등)에 마우스를 올리면 뜻을 풀어 보여주는 설명 (계산값으로 만든 문장, AI 호출 없음)
- 섹션 2 프롬프트 일반화: 칼국수·생면·말레이시아 예시와 '국물 생면을 경쟁재로 지정' 지시 제거 → 입력 제품과 같은 카테고리·용도의 경쟁 제품
- 검색 실패 안내: Tavily 크레딧 소진이면 섹션 2·3에 "검색 API 크레딧이 만료되어…", 키 오류·기타 실패도 구분. 0건 검색 결과는 캐시하지 않음
- 부스 스킬 문서(SKILL.md) 예시를 특정 식품군(밀키트)에 치우치지 않게 교체
- 부스 화면: Key Actions·KPI 숨김(AI 출력·results 파일에는 유지), 카드마다 '요약 | 원문' 스위치 하나(요약은 80자 이상 문장만),
  Execution Plan 구분선 목록 + 오른쪽 근거 뱃지, Visitor Journey = 제목("3분:" 자동 제거) → 🎯 목표 → 바이어에게 건넬 말(말풍선) → 👀·🙋·🧰 세 칸
