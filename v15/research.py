"""박람회 준비 데스크 리서치 (v9.0) — Tavily 조사는 보여주기만, 부스 컨셉은 OpenAI가 스스로 조사해서 기획

v9에서 바뀐 점
  - Tavily 조사(주요 사이트 우선 검색·발췌·요약 카드)는 사용자 화면에 보여주기만 하고, 부스 기획에는 넘기지 않는다.
  - 부스 컨셉은 plan_booth_independent(): 사용자 입력(제품·국가·박람회·기업 정보)만 받은 OpenAI가
    ① 웹 검색(Responses API web_search)으로 스스로 자료조사 → 조사 메모(R번호, 출처 URL)
       (웹 검색이 안 되면 모델 지식으로 대체하고 "AI 지식"으로 표시)
    ② 전략 뼈대 → ③ 초안 → ④ 채점 → ⑤ 수정 (v8과 같은 페르소나·스킬 문서)
  - Tavily 조사와 부스 기획은 동시에 실행한다.

(아래는 v8 설명)

v8에서 바뀐 점
  - skills/<이름>/ 의 스킬 문서(업무 방법·채점표·예시)를 OpenAI 시스템 프롬프트로 주입 (skill_loader.py)
    기본 스킬: skills/booth_marketing. 다른 스킬(예: Claude 마케팅 스킬 원문)은 폴더를 만들고 TAVILY_BOOTH_SKILL로 선택
  - 부스 기획을 단계로 쪼갬: ① 전략 뼈대(인사이트·타깃 바이어·빅 아이디어·RTB) → ② 기획안 초안
    → ③ 바이어 페르소나가 채점표로 항목별 1~5점 채점 → ④ 기준 점수 미만 항목이 있을 때만 수정
  - 기획안에 방문자 동선(3초·30초·3분)과 KPI 추가

(아래는 v7 설명)

v7에서 바뀐 점
  - plan_booth_concept(): 기획자 페르소나(15년차 박람회 마케터)가 조사 발췌를 근거로 부스 컨셉 초안을 쓰고
    → 바이어 페르소나(미국 대형 유통 MD 10년차)가 검토 → 기획자가 반영해 최종안 (B안, TAVILY_BOOTH_REVIEW=0이면 1회 생성 A안)
    6개 항목(메인 비주얼·슬로건·시그니처 페어링·시연·패키징·진열 가이드) + 핵심 요약·포지셔닝·리스크·기업 확인 질문
    불릿마다 근거 표시(조사 발췌 E번호 / 기업 입력 / 기획 제안). 근거 표시 외에는 코드가 문장을 지우거나 막지 않는다.
  - 주요 사이트 한정 검색에서 목록 밖 도메인이 섞여 오면 "일반 웹"으로 표시 (v6 버그 수정)
  - 질문 카드에서 결론과 똑같은 핵심 사실 문장 중복 제거
...
흐름
  ...
  7. plan_booth_concept() 페르소나 부스 컨셉 기획 (v7: 기획자 초안 → 바이어 검토 → 기획자 수정)
...
from constants.personas import PLANNER_PERSONA, REVIEWER_PERSONA
from skill_loader import load_skill, planner_system_prompt, reviewer_system_prompt
...
VERSION = "v14.0"              # 질문·프롬프트·규칙을 바꾸면 올려서 리포트 캐시 무효화
...
def get_clients():
    ...
    return (model_upgrade.wrap(OpenAI(api_key=openai_key, default_headers=SAFE_HEADERS, timeout=300)),
...
def _ask_json(client, prompt, temperature=0.0, model=None, system=None):
...
def _search(conn, tavily, query, params, ttl):
    ...
    if results:   # 0건은 저장하지 않는다 (크레딧 소진 등 일시적 문제가 30일간 굳지 않게)
...
CREDIT_ERROR_WORDS = ("usage limit", "exceeds your plan", "credit", "quota", "plan limit", "432", "433")


def search_error_kind(error):
    """Tavily 오류 종류: credits(크레딧 소진·한도 초과) · key(키 오류) · failed(그 밖)."""
    name = error.__class__.__name__
    text = str(error).lower()
    if name in ("UsageLimitExceededError", "TavilyKeylessLimitError") or any(w in text for w in CREDIT_ERROR_WORDS):
        return "credits"
    if name in ("InvalidAPIKeyError", "MissingAPIKeyError", "ForbiddenError") or "api key" in text or "401" in text:
        return "key"
    return "failed"
...
def collect_sources(conn, tavily, queries, ttl, diag, seen=None, sources=None):
    ...
    for q in queries:
        ...
        try:
            ...
            diag["_errors"][search_error_kind(e)] += 1
            continue
        for r in results:
            if r["url"] in seen:
                continue
            ...
            # v7: 주요 사이트 한정 검색이어도 Tavily가 목록 밖 도메인을 섞어 줄 때가 있어 코드로 다시 확인
            allowed = q["params"].get("include_domains")
            tier = q.get("tier", "open")
            if tier == "main" and allowed and not _domain_in(domain, allowed):
                tier = "open"
                diag[q["qid"]]["outside_main"] += 1
            sources.append({**r, "id": len(sources), "found_by": q["qid"], "domain": domain, "tier": tier,
...
JSON: {{"conclusion": "...", "conclusion_ids": ["E1"], "points": [{{"text": "...", "quote_ids": ["E1"]}}],
    ...
    # v7: 결론과 똑같은 문장은 핵심 사실 목록에서 뺀다 (화면에서 같은 문장이 두 번 보이지 않게)
    if card["conclusion"]:
        same = _norm(card["conclusion"]["text"])
        card["points"] = [p for p in card["points"] if _norm(p["text"]) != same]
    card["interpretation"] = data.get("interpretation") if card["points"] or card["conclusion"] else None
    card["gaps"] = data.get("gaps")
    return card


BOOTH_SECTIONS = {  # 키: (화면 제목, 설명)
    "main_visual": ("메인 비주얼", "Main Visual"),
    "slogan": ("슬로건", "Slogan"),
    "signature_pairing": ("시그니처 페어링", "Signature Pairing"),
    "demonstration": ("시연", "Live Demonstration"),
    "packaging": ("패키징", "Packaging Guide"),
    "merchandising": ("진열 가이드", "Merchandising Guide"),
}
BOOTH_REVIEW = os.getenv("TAVILY_BOOTH_REVIEW", "1") != "0"   # 0이면 A안(1회 생성)
BOOTH_SKILL = os.getenv("TAVILY_BOOTH_SKILL") or "booth_marketing"   # skills/ 아래 폴더 이름
REVISE_BELOW = int(os.getenv("TAVILY_BOOTH_REVISE_BELOW") or 4)     # 채점에서 이 점수 미만 항목이 있으면 수정
# v9: 부스 기획용 OpenAI 자체 조사
SELF_RESEARCH_WEB = os.getenv("TAVILY_V9_WEB_SEARCH", "1") != "0"      # 0이면 웹 검색 없이 모델 지식만
SELF_RESEARCH_MODEL = BOOTH_MODEL
MAX_RESEARCH_NOTES = 12
RESEARCH_TOPICS = {"consumer": "소비자", "competitor": "경쟁 제품", "buyer": "바이어·유통",
                   "exhibition": "박람회", "trend": "트렌드", "regulation": "규정·인증"}

BRIEF_SCHEMA = """{
  "insights": [{"fact": "사실(발췌 요지)", "insight": "그래서 사람들은 무엇을 원하나", "implication": "우리 부스가 할 일",
                "basis": ["R1"]}],
  "target_buyers": [{"priority": 1, "type": "바이어 유형", "job": "부스에서 확인하려는 것"}],
  "big_idea": {"line_en": "", "line_ko": "", "why": "왜 이 약속인가 (근거 요약)", "basis": ["기업:강점", "R3"]},
  "rtb": [{"text": "믿을 이유", "basis": []}]
}"""

BOOTH_SCHEMA = """{
  "summary": "핵심 전략 요약 1~2문장",
  "positioning": {"target_buyer": "", "core_message": ""},
  "main_visual": {"concept_en": "", "concept_ko": "", "key_message": "",
                  "bullets": [{"text": "", "basis": ["R1"]}]},
  "slogan": {"main_en": "", "main_ko": "", "sub_en": "", "sub_ko": "",
             "bullets": [{"text": "슬로건 선정 이유", "basis": ["기업:강점"]}]},
  "signature_pairing": {"item": "", "intent": "", "bullets": [{"text": "", "basis": ["R2"]}]},
  "demonstration": {"title": "", "bullets": [{"text": "", "basis": ["기획"]}]},
...
def _booth_facts(product_name, country, exhibition_name, cards, quote_index, profile):
    """AI에게 넘길 조사 자료. 발췌는 원문·번역·범위·시장·연도를 함께 준다."""
    quotes = [{"id": q["id"], "quote": q["quote"], "translation_ko": q["translation_ko"],
               "scope": q["scope_label"], "market": q["market_label"], "year": q["year"] or "미상",
               "source_tier": q.get("tier_label", "")}
              for q in quote_index.values()]
    summaries = [{"question": c["title"],
                  "conclusion": (c["conclusion"] or {}).get("text"),
                  "points": [p["text"] for p in c["points"]],
                  "notice": c.get("notice")} for c in cards]
    profile_text = "\n".join(f"- {k}: {v}" for k, v in profile.items() if v) or "(입력 없음)"
    has_product = any(c["product_quotes"] for c in cards)
    return f"""[제품] {product_name} / [시장] {country} / [박람회] {exhibition_name or "(미정)"}
[제품 자체 현지 자료] {"있음" if has_product else "없음 - 제품군·한국 식품 자료를 바탕으로 기획"}
...
[조사 발췌] (근거 표기: 발췌 id, 예: "E12")
{json.dumps(quotes, ensure_ascii=False, indent=1)}"""


BOOTH_RULES = """작성 규칙:
- 6개 항목(메인 비주얼, 슬로건, 시그니처 페어링, 시연, 패키징, 진열 가이드)을 모두 채우세요.
- 각 불릿의 basis에 근거를 적으세요: 조사 메모 id("R3"), 기업 정보("기업:강점"), 근거 없는 아이디어는 "기획".
  조사 메모를 활용할 수 있는 곳은 적극적으로 활용하세요.
- 슬로건은 바이어 언어(영어 또는 현지어) 원문과 한국어 번역을 함께. 불릿 설명은 한국어.
...
def _clean_bullets(items, quote_index, profile):
    """불릿 정리 + 근거 분류. 문장은 지우지 않고, 근거 표기만 확인해서 라벨을 붙인다."""
    out = []
    if isinstance(items, (str, dict)):  # 불릿 목록 대신 문자열·객체 하나만 온 경우
        items = [items]
    for b in items or []:
        if isinstance(b, str):
            b = {"text": b, "basis": []}
...
def _classify_basis(basis, quote_index, profile):
    quote_ids, company = [], []
    for token in basis if isinstance(basis, list) else [basis]:
        t = str(token or "").strip()
        m = re.fullmatch(r"[ER]\d+", t)
        if m and t in quote_index:
            quote_ids.append(t)
        elif t.startswith("기업:") and profile.get(t[3:].strip()):
            company.append(t[3:].strip())
    quote_ids = list(dict.fromkeys(quote_ids))
    company = list(dict.fromkeys(company))
    return {"quote_ids": quote_ids, "company": company, "is_idea": not quote_ids and not company}


def normalize_booth(data, quote_index, profile):
    """AI 응답을 화면용 구조로 정리한다. 빠진 항목은 빈 값으로 둔다."""
    ...
    sections = {}
    for key in BOOTH_SECTIONS:
        raw = data.get(key) if isinstance(data.get(key), dict) else {}
        sec = {k: text(raw, k) for k in ("concept_en", "concept_ko", "key_message", "main_en", "main_ko",
                                          "sub_en", "sub_ko", "item", "intent", "title", "direction") if raw.get(k)}
        sec["bullets"] = _clean_bullets(raw.get("bullets"), quote_index, profile)
        if key == "merchandising":
            sec["zones"] = [{"name": text(z, "name"), "position": text(z, "position"), "purpose": text(z, "purpose"),
                             **_classify_basis(z.get("basis"), quote_index, profile)}
                            for z in raw.get("zones") or [] if isinstance(z, dict) and text(z, "name")]
        sections[key] = sec
    pos = data.get("positioning") if isinstance(data.get("positioning"), dict) else {}
    return {
...
# ---------------------------------------------------------------
SHORT_MIN_CHARS = 80     # 이보다 짧은 문장은 요약하지 않음 (요약과 원문이 거의 같아지는 것 방지)
...
STAGE_PREFIX = re.compile(r"^\s*(?:\d+\s*(?:초|분|s|sec|min)|3s|30s|3min)\s*[:：·\-–—]\s*", re.IGNORECASE)


def normalize_visitor_flow(flow):
    """3초·30초·3분 단계별 {headline, goal, visitor, staff, props, message}. 예전 형식(문장 하나)도 headline으로 받는다."""
    flow = flow if isinstance(flow, dict) else {}
    out = {}
    for key in ("3s", "30s", "3min"):
        stage = flow.get(key)
        if isinstance(stage, str):
            stage = {"headline": stage}
        stage = stage if isinstance(stage, dict) else {}
        headline = STAGE_PREFIX.sub("", str(stage.get("headline") or "").strip())   # 탭 이름과 겹치는 "3분:" 제거
        out[key] = {"headline": headline, "goal": str(stage.get("goal") or "").strip(),
                    "visitor": _str_list(stage.get("visitor")), "staff": _str_list(stage.get("staff")),
                    "props": _str_list(stage.get("props")), "message": str(stage.get("message") or "").strip()}
    return out


def normalize_brief(data, quote_index, profile):
    """전략 뼈대 정리. 문장은 지우지 않고 근거 표시만 분류한다."""
    data = data if isinstance(data, dict) else {}

    def text(obj, key):
        return str((obj or {}).get(key) or "").strip()

    insights = [{"fact": text(i, "fact"), "insight": text(i, "insight"), "implication": text(i, "implication"),
                 **_classify_basis(i.get("basis"), quote_index, profile)}
                for i in data.get("insights") or [] if isinstance(i, dict) and text(i, "insight")][:4]
    buyers = [{"priority": b.get("priority"), "type": text(b, "type"), "job": text(b, "job")}
              for b in data.get("target_buyers") or [] if isinstance(b, dict) and text(b, "type")][:3]
    big = data.get("big_idea") if isinstance(data.get("big_idea"), dict) else {}
    return {
        "insights": insights,
        "target_buyers": buyers,
        "big_idea": {"line_en": text(big, "line_en"), "line_ko": text(big, "line_ko"), "why": text(big, "why"),
                     **_classify_basis(big.get("basis"), quote_index, profile)},
        "rtb": _clean_bullets(data.get("rtb"), quote_index, profile)[:4],
    }


def normalize_scores(review, criteria):
    """채점 결과 정리: 채점표에 있는 항목만, 1~5 정수만 인정."""
    raw = (review or {}).get("scores") or {}
    scores = []
    for c in criteria:
        item = raw.get(c["id"]) if isinstance(raw, dict) else None
        try:
            score = int((item or {}).get("score"))
        except (TypeError, ValueError, AttributeError):
            score = None
        if score is not None and not 1 <= score <= 5:
            score = None
        scores.append({"id": c["id"], "label": c["label"], "score": score,
                       "reason": str((item or {}).get("reason") or "").strip() if isinstance(item, dict) else ""})
    return scores
...
def plan_booth_concept(client, product_name, country, exhibition_name, cards, quote_index, profile, facts=None,
    ...
    ① 전략 뼈대(인사이트·타깃 바이어·빅 아이디어·RTB) → ② 기획안 초안 → (B안) ③ 채점표로 항목별 채점
    → ④ 기준 점수 미만 항목이 있으면 수정. 뒤 단계가 실패하면 앞 단계 결과를 쓴다."""
    skill = load_skill(BOOTH_SKILL)
    planner = planner_system_prompt(PLANNER_PERSONA, skill)
    reviewer = reviewer_system_prompt(REVIEWER_PERSONA, skill)
    if facts is None:
        facts = _booth_facts(product_name, country, exhibition_name, cards, quote_index, profile)

    step = progress or (lambda label: None)
    step("전략 뼈대 (인사이트·빅 아이디어)")
    # ① 전략 뼈대: 실행안을 쓰기 전에 생각의 순서(스킬 1~4단계)를 따로 끝내게 한다
    try:
        brief = _ask_json(client, f"""스킬의 1~4단계(사실→인사이트→시사점, 타깃 바이어, 빅 아이디어, 믿을 이유)만 먼저 수행하세요.
아직 부스 실행안(비주얼·슬로건 등)은 쓰지 마세요.
{GUARD}

{facts}

규칙: 인사이트 2~4개(서로 다른 질문에서), 타깃 바이어 1~2순위, 빅 아이디어는 한 문장(바이어 언어 + 한국어),
믿을 이유 2~4개. 근거는 basis에 조사 메모 id("R3"), 기업 정보("기업:강점"), 근거 없으면 "기획". 설명은 한국어.
JSON 형식:
{BRIEF_SCHEMA}""", temperature=0.4, model=BOOTH_MODEL, system=planner)
    except Exception as e:
        print(f"전략 뼈대 실패 (바로 기획안 작성): {e}")
        brief = {}
    brief_block = (f"[전략 뼈대 - 모든 실행 항목은 아래 빅 아이디어를 증명해야 합니다]\n"
                   f"{json.dumps(brief, ensure_ascii=False, indent=1)}") if brief else ""

    # ② 기획안 초안
    step("부스 기획안 초안 작성")
    try:
        draft = _ask_json(client, f"""아래 조사 결과와 전략 뼈대를 바탕으로 {country} 박람회 부스 컨셉 기획안을 작성하세요.
스킬의 5~9단계(6개 실행 항목, 3초·30초·3분 동선, KPI, 자료가 부족할 때, 자가 점검)를 따르세요.
{GUARD}
...
{BOOTH_SCHEMA}""", temperature=0.6, model=BOOTH_MODEL, system=planner)
        ...
        print(f"부스 컨셉 초안 실패: {e}")
        return None

    review, final, stage, scores = None, draft, "draft", []
    if BOOTH_REVIEW:
        # ③ 채점표로 채점
        step("바이어 페르소나 채점")
        try:
            ids = ", ".join(c["id"] for c in skill["criteria"])
            review = _ask_json(client, f"""아래는 {product_name}의 {country} 박람회 부스 기획 초안입니다.
...
JSON: {{"scores": {{"insight": {{"score": 4, "reason": ""}}}}, "overall": "", "comments": [{{"section": "", "issue": "", "suggestion": ""}}]}}""",
                               temperature=0.2, model=BOOTH_MODEL, system=reviewer)
        except Exception as e:
            print(f"부스 컨셉 채점 실패 (초안 사용): {e}")
            review = None
        scores = normalize_scores(review, skill["criteria"])
        low = [x for x in scores if x["score"] is not None and x["score"] < REVISE_BELOW]

        # ④ 기준 미만 항목이 있을 때만 수정 (모두 기준 이상이면 호출을 아낀다)
        step("채점 반영 수정" if review and low else "기준 통과 · 수정 생략")
        if review and low:
            try:
                revised = _ask_json(client, f"""당신이 쓴 부스 기획 초안이 채점표로 평가되었습니다. 점수가 낮은 항목을 중심으로 고쳐 최종안을 쓰세요.
...
{BOOTH_SCHEMA[:-1]},
  "applied": [""]
    ...
    booth.update({
        "brief": normalize_brief(brief, quote_index, profile),
        "mode": "B" if BOOTH_REVIEW else "A",
        "stage": stage,
        "revise_below": REVISE_BELOW,
        "scores": scores,
        "skill": {"name": skill["name"], "folder": skill["folder"], "files": skill["files"],
                  "chars": skill["chars"], "fingerprint": skill["fingerprint"], "warnings": skill["warnings"]},
        "planner_persona": PLANNER_PERSONA,
        "reviewer_persona": REVIEWER_PERSONA if BOOTH_REVIEW else None,
        "review": {"overall": str(review.get("overall") or "").strip(),
                   "comments": [{"section": BOOTH_SECTIONS.get(c.get("section"), (c.get("section") or "전체",))[0],
...
# ---------------------------------------------------------------
# v9. 부스 기획용 OpenAI 자체 조사 (Tavily 조사와 분리)
# ---------------------------------------------------------------
def _extract_json(text):
    """모델 답변에서 JSON 객체를 꺼낸다 (웹 검색 응답은 JSON 모드를 쓸 수 없어 본문에서 찾는다)."""
    text = (text or "").strip()
...
def _country_iso2(country):
    info = COUNTRIES.get(_country_key(country))
    if not info:
        return None
    return {"uk": "GB"}.get(info[0], info[0].upper())


def _inputs_block(product_name, country, exhibition_name, exhibition_website, profile):
    profile_text = "\n".join(f"- {k}: {v}" for k, v in profile.items() if v) or "(입력 없음)"
    return f"""[제품] {product_name} / [시장] {country}
[박람회] {exhibition_name or "(미정)"}{f" ({exhibition_website})" if exhibition_website else ""}

[기업 제품 정보] (근거 표기: "기업:항목명")
{profile_text}"""


def _research_prompt(inputs, with_web):
    how = ("웹 검색으로 최신 자료를 직접 찾아보세요. 각 메모에는 실제로 참고한 페이지의 URL을 source_url에 적으세요."
           if with_web else "알고 있는 지식으로 정리하세요. 확실하지 않은 수치·브랜드는 쓰지 마세요. source_url은 빈 문자열.")
    topics = ", ".join(f"{k}({v})" for k, v in RESEARCH_TOPICS.items())
    return f"""박람회 부스 컨셉을 기획하기 전에 필요한 자료조사를 하세요. {how}

{inputs}

조사할 것 (부스 기획에 쓸 수 있는 실무 정보 위주):
- 이 제품(또는 제품군)을 현지 소비자는 언제·왜 먹고, 무엇을 아쉬워하는가
- 현지에서 팔리는 경쟁 제품(브랜드·가격·메시지)
- 바이어·유통사가 수입 제품에서 확인하는 것(인증, 라벨, 유통기한, 채널)
- 이 박람회의 성격·방문자·최근 트렌드, 부스 운영·시식 관련 규칙
메모 6~{MAX_RESEARCH_NOTES}개. topic은 {topics} 중 하나. text는 한국어 한두 문장(수치·브랜드는 원문 표기 유지).

JSON만 답하세요:
{{"notes": [{{"topic": "consumer", "text": "", "source_url": "", "source_title": ""}}]}}"""


def _collect_citations(response):
    """Responses API 답변의 url_citation 주석에서 실제로 인용된 URL을 모은다."""
    urls = {}
...
def _normalize_notes(data, citations, method):
    notes = []
    for n in (data or {}).get("notes") or []:
        if not isinstance(n, dict) or not str(n.get("text") or "").strip():
            continue
        url = str(n.get("source_url") or "").strip()
        if method == "knowledge" or not url:
            status = "AI 지식"
        elif url in citations or any(url.rstrip("/") == c.rstrip("/") for c in citations):
            status = "웹 출처"
        else:
            status = "출처 미확인"   # 모델이 적은 URL이 실제 검색 인용에 없음
        topic = n.get("topic") if n.get("topic") in RESEARCH_TOPICS else "trend"
        notes.append({"id": f"R{len(notes) + 1}", "topic": topic, "topic_label": RESEARCH_TOPICS[topic],
                      "text": str(n["text"]).strip(), "source_url": url if method != "knowledge" else "",
...
def openai_self_research(client, product_name, country, exhibition_name, exhibition_website, profile):
    """OpenAI 스스로 자료조사. 웹 검색이 되면 웹 검색, 안 되면 모델 지식.
    반환: {"method": web_search|knowledge|failed, "notes": [...], "citations": [...], "error": str}"""
    inputs = _inputs_block(product_name, country, exhibition_name, exhibition_website, profile)
    error = ""
    if SELF_RESEARCH_WEB:
        ...
        try:
            response = client.responses.create(
                model=SELF_RESEARCH_MODEL, tools=[tool], instructions=PLANNER_PERSONA,
                input=_research_prompt(inputs, with_web=True))
            citations = _collect_citations(response)
            notes = _normalize_notes(_extract_json(getattr(response, "output_text", "")), citations, "web_search")
            if notes:
                return {"method": "web_search", "notes": notes,
                        "citations": [{"url": u, "title": t} for u, t in citations.items()], "error": ""}
            error = "웹 검색 답변에서 조사 메모를 찾지 못함"
        except Exception as e:
            error = f"웹 검색 실패: {e}"
            print(f"OpenAI 웹 검색 조사 실패 (모델 지식으로 대체): {e}")
    try:
        data = _ask_json(client, _research_prompt(inputs, with_web=False), temperature=0.3,
                         model=SELF_RESEARCH_MODEL, system=PLANNER_PERSONA)
        return {"method": "knowledge", "notes": _normalize_notes(data, {}, "knowledge"), "citations": [],
                "error": error}
    except Exception as e:
        print(f"OpenAI 자체 조사 실패: {e}")
        return {"method": "failed", "notes": [], "citations": [], "error": f"{error} / 지식 정리 실패: {e}".strip(" /")}


def plan_booth_independent(client, product_name, country, exhibition_name, exhibition_website, profile, progress=None):
    """Tavily 조사를 쓰지 않는 부스 기획: 사용자 입력 → OpenAI 자체 조사 → v8과 같은 기획 단계."""
    if progress:
        progress("부스용 자체 웹 조사 (OpenAI)")
    research = openai_self_research(client, product_name, country, exhibition_name, exhibition_website, profile)
    note_index = {n["id"]: n for n in research["notes"]}
    notes_for_ai = [{"id": n["id"], "topic": n["topic_label"], "text": n["text"], "source": n["source_status"]}
                    for n in research["notes"]]
    facts = (_inputs_block(product_name, country, exhibition_name, exhibition_website, profile)
             + "\n\n[자체 조사 메모] (근거 표기: 메모 id, 예: \"R3\")\n"
             + (json.dumps(notes_for_ai, ensure_ascii=False, indent=1) if notes_for_ai else "(조사 메모 없음)"))
    booth = plan_booth_concept(client, product_name, country, exhibition_name, [], note_index, profile, facts=facts,
                               progress=progress)
    if booth is None:
        return None
    booth["research"] = research
    booth["source_mode"] = "openai_self_research"
    booth["has_product_evidence"] = None   # v9는 Tavily 발췌를 쓰지 않으므로 해당 없음
    return booth
...
def _overall_search_error(sources, failed, diag):
    """검색이 모두 실패해 출처가 하나도 없을 때만 오류 종류를 돌려준다 (화면 안내용). 크레딧 > 키 > 기타 순."""
    if sources or not failed:
        return None
    errors = diag.get("_errors") or {}
    for kind in ("credits", "key", "failed"):
        if errors.get(kind):
            return kind
    return "failed"


RESEARCH_STEPS = 10   # 진행률 표시용 단계 수 (아래 step() 호출 횟수)


def run_research(product_name, country, *, exhibition_name=None, exhibition_website=None,
                 company_profile=None, db_path=None, force=False, include_booth=True, progress=None):
    """반환 데이터 구조는 README '결과 데이터 구조' 참고.
    v12: include_booth=False면 부스 기획을 돌리지 않는다(대시보드가 따로 실행). progress(label)는 단계마다 호출."""
    step = progress or (lambda label: None)
    ...
    # 스킬 문서를 고치면(지문 변경) 리포트 캐시도 새로 만든다
    key = _hash({"v": VERSION, "m": MODEL, "bm": BOOTH_MODEL, "br": BOOTH_REVIEW, "rb": REVISE_BELOW,
                 "sw": SELF_RESEARCH_WEB, "sm": SELF_RESEARCH_MODEL,
                 "sk": load_skill(BOOTH_SKILL)["fingerprint"], "p": product_name, "c": country,
    ...
    with _locks[key]:
        ...
        try:
            ...
            try:
                ...
                # v9: 부스 기획은 사용자 입력만으로 따로 돌린다 (Tavily 조사 결과를 넘기지 않음)
                booth_pool = booth_future = None
                if include_booth:
                    booth_pool = ThreadPoolExecutor(max_workers=1)
                    booth_future = booth_pool.submit(plan_booth_independent, client, product_name, country,
                                                     exhibition_name, exhibition_website, profile)
                step("현지 검색어 준비")
                ...
                open_queries = []
                step("부족한 질문 일반 웹 보강" if weak else "보강 검색 생략 (자료 충분)")
                if weak:
                ...
                booth, booth_error = None, None
                if booth_future:
                    try:
                        booth = booth_future.result()
                    except Exception as e:
                        print(f"부스 기획 실패: {e}")
                        booth, booth_error = None, str(e)
                    booth_pool.shutdown(wait=False)
            except Exception:
            ...
            result = {
                ...
                "cards": cards, "booth": booth, "booth_error": booth_error, "quotes": quotes,
                ...
                "failed_queries": failed, "search_error": _overall_search_error(sources, failed, diag),
                "fetched_at": _now(), "from_cache": False, "version": VERSION,
            }
            if quotes:
