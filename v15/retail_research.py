...
def analyze_retail_market(product_name: str, country: str, strengths: str, raw_materials: str, target_price_str: str,
    ...
    2. 경쟁 제품 선정 (★입력 제품 기준):
       - 반드시 [우리 제품명]과 같은 카테고리·같은 용도(먹는 상황, 조리 방식, 제형)로 {country} 현지에서 실제로 팔리는 대표 제품 1개를 1위 경쟁재로 지정하세요.
       - 이름이 비슷해도 카테고리나 용도가 다른 제품은 고르지 마세요. 현지 동급 제품이 없으면 가장 가까운 대체재를 고르고 competitor_specs에 그 이유를 적으세요.
    3. 바이어 피칭 (제품 고유 팩트 기반):
       - "소비자 니즈 충족" 같은 모호한 문구 금지.
       - [우리 제품 원료]와 [제품 강점]을 근거로, 바이어 매대의 객단가·구색을 넓히는 논리로 작성하세요.
    4. retail_domains:
       - {country} 현지 대형 유통몰(대형마트·식료품 온라인몰) 도메인 2개.
    ...
    {{
      ...
      "category_anchors": "검색에 반드시 포함할 카테고리 단어 (현지어)",
      "negative_anchors": "검색에서 제외할 다른 카테고리 단어 (예: -단어1 -단어2)",
      "min_price": 5.0,
      "max_price": 7.0,
      "currency_symbol": "{target_currency}",
      "competitor_product": "현지 1위 동급 경쟁 제품명 (원어 및 한글)",
      "competitor_specs": "경쟁 제품의 실제 규격 (용량·포장 단위·제형·보관 방식)",
      "consumer_complaints": "{country} 현지 소비자가 이 제품군에서 느끼는 구체적 불만",
      "shelf_location": "현지 마트 내 이 제품군이 놓이는 정확한 매대 명칭 (현지어 또는 영어)",
      "value_pitch": "우리 제품의 원료와 강점을 담은 바이어 입점 피칭"
    }}
    반드시 순수 JSON만 반환하세요.
    """
    ...
    search_error = None   # Tavily 실패 종류 (credits·key·failed) — 화면 안내용
    ...
    try:
        ...
        search_success = False
        from research import search_error_kind   # 같은 판별 규칙 사용
        search_error = search_error_kind(e)
    ...
    final_result = {
        ...
        "retail_price": {
            ...
            "method": price_method,
            "search_error": search_error if price_method == "estimate" else None,
