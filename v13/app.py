#!/usr/bin/env python3
"""페어메이트 박람회 준비 — 트렌드 조사 탭 & 부스 컨셉 기획 (v13).
...
def _trend_page(form, data=None, error=None, force=False, booth_job_id=None):
...
def trend_result(job_id):
    ...
    if not job or job["status"] == "running":
        return redirect("/")
    return _trend_page(job["form"], job["result"], job["error"], job["force"], job["links"].get("booth_job_id"))
