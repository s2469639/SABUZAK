"""네트워크 없이 v10b 대시보드(v10 + 상위 OpenAI 모델)(4개 섹션 결합)와 화면을 검증하는 오프라인 테스트.
...
class DashboardTest(unittest.TestCase):
    ...
    def test_render_dashboard(self):
        ...
        for text in ["● 1. 연관 검색어 기반 시장 트렌드 4단계 클러스터링", "CULTURE TRIGGER", "kdrama yakgwa",
                     ...
                     "● 4. 부스 컨셉 기획", "OpenAI 단독 기획 · gpt-6-astra", "추론 깊이 medium", "섹션 1~3 모델: gpt-5.6-terra (low)", "BIG IDEA", "MAIN VISUAL", "VISITOR FLOW",
...
class FakeEndpoint:
    def __init__(self, missing=(), reject=None):
        self.calls, self.missing, self.reject = [], set(missing), reject

    def create(self, **kw):
        self.calls.append(kw)
        req = httpx.Request("POST", "https://api.openai.com/v1/x")
        if kw["model"] in self.missing:
            raise openai.NotFoundError("model not found", response=httpx.Response(404, request=req),
                                       body={"code": "model_not_found"})
        if self.reject and self.reject in kw:
            err = openai.BadRequestError("unsupported", response=httpx.Response(400, request=req), body={})
            err.param = self.reject
            raise err
        return SimpleNamespace(model=kw["model"])


def fake_openai(**kw):
    chat = FakeEndpoint(**kw)
    responses = FakeEndpoint(**kw)
    return SimpleNamespace(chat=SimpleNamespace(completions=chat), responses=responses, models="models-api"), chat, responses
...
class ModelUpgradeTest(unittest.TestCase):
    ...
    def test_legacy_names_mapped_and_reasoning_params(self):
        raw, chat, responses = fake_openai()
        client = model_upgrade.wrap(raw)
        client.chat.completions.create(model="gpt-4o-mini", temperature=0.2, messages=[])
        client.chat.completions.create(model="gpt-4o", temperature=0.6, messages=[])
        client.responses.create(model="gpt-4o", tools=[{"type": "web_search"}], input="x")
        self.assertEqual(chat.calls[0], {"model": "gpt-5.6-terra", "reasoning_effort": "low", "messages": []})
        self.assertEqual(chat.calls[1], {"model": "gpt-6-astra", "reasoning_effort": "medium", "messages": []})
        self.assertEqual(responses.calls[0]["reasoning"], {"effort": "medium"})
        self.assertEqual(client.models, "models-api")             # 나머지는 원래 클라이언트로

    def test_non_reasoning_model_keeps_temperature(self):
        raw, chat, _ = fake_openai()
        with mock.patch.object(model_upgrade, "TASK_MODEL", "gpt-4.1"):
            model_upgrade.wrap(raw).chat.completions.create(model="gpt-4o-mini", temperature=0.2, messages=[])
        self.assertEqual(chat.calls[0], {"model": "gpt-4.1", "temperature": 0.2, "messages": []})

    def test_missing_model_falls_back_and_is_remembered(self):
        raw, chat, _ = fake_openai(missing={"gpt-6-astra"})
        client = model_upgrade.wrap(raw)
        r = client.chat.completions.create(model="gpt-4o", messages=[])
        self.assertEqual(r.model, "gpt-5.6-sol")
        client.chat.completions.create(model="gpt-4o", messages=[])
        self.assertEqual([c["model"] for c in chat.calls], ["gpt-6-astra", "gpt-5.6-sol", "gpt-5.6-sol"])
        self.assertEqual(model_upgrade.models_used()["booth"], "gpt-5.6-sol")

    def test_rejected_param_dropped(self):
        raw, chat, _ = fake_openai(reject="reasoning_effort")
        r = model_upgrade.wrap(raw).chat.completions.create(model="gpt-4o-mini", messages=[])
        self.assertEqual(r.model, "gpt-5.6-terra")
        self.assertNotIn("reasoning_effort", chat.calls[-1])

    def test_all_sections_use_wrapped_clients(self):
        self.assertEqual(pipeline.tavily_research.BOOTH_MODEL, "gpt-6-astra")
        self.assertEqual(pipeline.tavily_research.MODEL, "gpt-5.6-terra")
        self.assertEqual(trend_adapter.trend_usp.DEFAULT_LIGHT_MODEL, "gpt-5.6-terra")
        with mock.patch.dict(os.environ, {"OPENAI_API_KEY": "x", "TAVILY_API_KEY": "y"}):
            self.assertIsInstance(pipeline.tavily_research.get_clients()[0], model_upgrade.UpgradedOpenAI)
            self.assertIsInstance(trend_adapter.trend_usp._get_openai_client(), model_upgrade.UpgradedOpenAI)
            trend_adapter._retail_module = None
            self.assertIsInstance(trend_adapter._retail().openai_client, model_upgrade.UpgradedOpenAI)
...
class UnitTest(unittest.TestCase):
    ...
    def test_real_j_test_module_loads_with_safe_client(self):
        ...
        self.assertEqual(module.openai_client._client._custom_headers.get("Accept-Encoding"), "gzip, deflate")
