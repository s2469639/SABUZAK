#!/usr/bin/env python3
"""박람회 준비 데스크 리서치 CLI (v9.0, Tavily 조사는 보여주기만 + OpenAI 자체 조사 기반 부스 컨셉).
...
def main():
    ...
    b = r["booth"]
    if r.get("booth_error"):
        print(f"\n⚠️  부스 컨셉 기획 실패: {r['booth_error']}")
    if b:
        print("\n" + "=" * 70)
        steps = {"revised": "전략 뼈대 → 초안 → 채점 → 수정", "passed": "전략 뼈대 → 초안 → 채점(기준 통과)",
                 "draft": "전략 뼈대 → 초안"}
        print(f"■ 부스 컨셉 기획안 ({steps[b['stage']]}, {b['model']}, 스킬: {b['skill']['name']})")
        print("=" * 70)
        rs = b["research"]
        method = {"web_search": "웹 검색", "knowledge": "모델 지식", "failed": "조사 실패"}[rs["method"]]
        print(f"  (Tavily 조사 결과는 쓰지 않음) OpenAI 자체 조사: {method}, 메모 {len(rs['notes'])}건")
        if rs["error"]:
            print(f"    ⚠️  {rs['error']}")
        for n in rs["notes"]:
            src = n["source_url"] or "출처 없음"
            print(f"    [{n['id']}] ({n['topic_label']}, {n['source_status']}) {n['text']}  🔗 {src}")
        ...
        for w in b["skill"]["warnings"]:
            print(f"  ⚠️  {w}")
        br = b["brief"]
        if br["big_idea"]["line_en"]:
            print(f"\n  빅 아이디어: {br['big_idea']['line_en']} / {br['big_idea']['line_ko']}{basis(br['big_idea'])}")
        for i in br["insights"]:
            print(f"    인사이트: {i['fact']} → {i['insight']} → {i['implication']}{basis(i)}")
        for t in br["target_buyers"]:
            print(f"    타깃 바이어 {t['priority'] or ''}: {t['type']} ({t['job']})")
        for x in br["rtb"]:
            print(f"    믿을 이유: {x['text']}{basis(x)}")
        ...
        if any(flow.values()):
            print("\n  방문자 동선")
            for k, label in (("3s", "3초"), ("30s", "30초"), ("3min", "3분")):
                if flow[k]:
                    print(f"    {label}: {flow[k]}")
        if b["kpis"]:
            print("\n  KPI: " + " / ".join(b["kpis"]))
        if b["scores"]:
            print(f"\n  바이어 채점 (초안, {b['revise_below']}점 미만이면 수정)")
            for x in b["scores"]:
                print(f"    {x['label']}: {x['score'] if x['score'] is not None else '-'}/5 {x['reason']}")
        if b["risks"]:
            print("\n  확인할 점")
            for x in b["risks"]:
                print(f"    - {x['text']}{basis(x)}")
