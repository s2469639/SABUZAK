"""네트워크 없이 v9(Tavily 조사 표시 + OpenAI 자체 조사 기반 부스 컨셉) 로직을 검증하는 오프라인 테스트.
...
class V6PipelineTest(unittest.TestCase):
    def setUp(self):
        ...
        self.patches = [
            mock.patch.object(research, "get_clients", return_value=(FakeOpenAI(), self.tavily)),
    ...
    def test_flask_render(self):
        ...
        for text in ["v9 · Tavily 조사 + OpenAI 자체 조사 부스 컨셉", "검색 범위", "부스 컨셉 기획안", "1. 메인 비주얼",
                     "빅 아이디어", "Honey pastry that won&#39;t stick to your fingers", "방문자 동선", "바이어 채점",
                     "성과 지표 (KPI)", "스킬: booth_marketing",
...
class V7BoothTest(V6PipelineTest):
    ...
    def test_b_flow_draft_review_revise(self):
        ...
        self.assertEqual([c[0] for c in BOOTH_CALLS], ["brief", "draft", "review", "revise"])  # 3점 항목 → 수정
        self.assertTrue(all(c[1] == research.BOOTH_MODEL for c in BOOTH_CALLS))
        systems = {c[0]: c[2] for c in BOOTH_CALLS}
        for step in ("brief", "draft", "revise"):   # 기획자 = 페르소나 + 스킬 업무 방법 + 예시
            self.assertTrue(systems[step].startswith(research.PLANNER_PERSONA))
            self.assertIn("# 업무 방법 (스킬)", systems[step])
            self.assertIn("빅 아이디어 (Single-minded proposition)", systems[step])
            self.assertIn("# 좋은 예시", systems[step])
        self.assertTrue(systems["review"].startswith(research.REVIEWER_PERSONA))   # 검토자 = 페르소나 + 채점표
        self.assertIn("# 채점표", systems["review"])
        self.assertIn("single_minded | 빅 아이디어 일관성", systems["review"])
        b = r["booth"]
        self.assertEqual([x["id"] for x in b["scores"]],
                         ["insight", "single_minded", "buyer_value", "specificity", "distinctiveness", "feasibility"])
        self.assertEqual(b["scores"][1]["score"], 3)
        self.assertEqual(b["brief"]["big_idea"]["company"], ["강점"])
        self.assertEqual(len(b["brief"]["insights"]), 1)
        self.assertEqual(b["visitor_flow"]["30s"], "커피와 한입 시식")
        self.assertEqual(b["kpis"], ["바이어 명함 80장", "샘플 요청 20건"])
        self.assertEqual(b["skill"]["name"], "booth_marketing")
        b = r["booth"]
    ...
    def test_basis_classification_keeps_every_sentence(self):
        ...
        self.assertEqual(mv[0]["quote_ids"], ["R2"])   # 없는 R99, Tavily 발췌 E1은 빠지고 문장은 유지
        ...
        self.assertEqual(zones[0]["quote_ids"], ["R1"])
    ...
    def test_a_mode_single_call(self):
        with mock.patch.object(research, "BOOTH_REVIEW", False):
            r = self.run_with_profile()
        self.assertEqual([c[0] for c in BOOTH_CALLS], ["brief", "draft"])
...
class V8SkillTest(V6PipelineTest):
    """v8: 스킬 로더, 채점 기준 통과 시 수정 생략, 스킬 변경 시 캐시 갱신."""

    def setUp(self):
        super().setUp()
        BOOTH_CALLS.clear()
        self.saved_scores = dict(REVIEW_SCORES)

    def tearDown(self):
        REVIEW_SCORES.clear()
        REVIEW_SCORES.update(self.saved_scores)
        super().tearDown()

    def test_all_scores_pass_skips_revision(self):
        REVIEW_SCORES.update({k: 4 for k in REVIEW_SCORES})
        r = run_research("약과", "United States", db_path=self.db)
        self.assertEqual([c[0] for c in BOOTH_CALLS], ["brief", "draft", "review"])
        self.assertEqual(r["booth"]["stage"], "passed")
        self.assertTrue(r["booth"]["summary"].startswith("draft"))

    def test_revise_threshold_env(self):
        with mock.patch.object(research, "REVISE_BELOW", 3):      # 3점 미만만 수정 → 3점은 통과
            r = run_research("약과", "United States", db_path=self.db)
        self.assertEqual(r["booth"]["stage"], "passed")

    def test_brief_failure_still_plans(self):
        def failing(client, prompt, temperature=0.0, model=None, system=None):
            if "스킬의 1~4단계" in prompt:
                raise RuntimeError("timeout")
            if "부스 컨셉 기획안을 작성하세요" in prompt:
                BOOTH_CALLS.append(("draft", model, system))
                return booth_payload("draft")
            return fake_ask_json(client, prompt, temperature, model, system)
        with mock.patch.object(research, "_ask_json", side_effect=failing):
            r = run_research("약과", "United States", db_path=self.db)
        self.assertEqual(r["booth"]["brief"]["insights"], [])
        self.assertTrue(r["booth"]["summary"])

    def test_custom_skill_folder(self):
        with tempfile.TemporaryDirectory() as d:
            os.makedirs(os.path.join(d, "claude_marketing", "references"))
            with open(os.path.join(d, "claude_marketing", "SKILL.md"), "w", encoding="utf-8") as f:
                f.write("---\nname: campaign-planner\ndescription: test\n---\n# 캠페인 기획\n"
                        "1. 타깃을 정한다\nUse the Read tool to open brief.md\n2. 메시지를 정한다\n")
            with open(os.path.join(d, "claude_marketing", "references", "voice.md"), "w", encoding="utf-8") as f:
                f.write("# 브랜드 보이스\n짧고 단정하게")
            # 채점표가 없는 스킬은 기본 스킬(booth_marketing)의 채점표를 쓰므로 그것도 임시 폴더에 복사
            real = skill_loader.SKILLS_DIR
            with mock.patch.object(skill_loader, "SKILLS_DIR", d):
                os.makedirs(os.path.join(d, "booth_marketing"))
                with open(os.path.join(real, "booth_marketing", "rubric.md"), encoding="utf-8") as src, \
                     open(os.path.join(d, "booth_marketing", "rubric.md"), "w", encoding="utf-8") as dst:
                    dst.write(src.read())
                skill = skill_loader.load_skill("claude_marketing")
        self.assertEqual(skill["name"], "campaign-planner")
        self.assertNotIn("Read tool", skill["instructions"])          # Claude 전용 도구 지시 제거
        self.assertIn("2. 메시지를 정한다", skill["instructions"])
        self.assertIn("짧고 단정하게", skill["references"])
        self.assertEqual(len(skill["criteria"]), 6)                     # 채점표 없음 → 기본 채점표
        self.assertTrue(any("채점표" in w for w in skill["warnings"]))
        self.assertIn("references/voice.md", skill["files"])

    def test_missing_skill_falls_back(self):
        skill = skill_loader.load_skill("does_not_exist")
        self.assertEqual(skill["folder"], "booth_marketing")
        self.assertTrue(any("찾지 못해" in w for w in skill["warnings"]))

    def test_skill_budget_truncates(self):
        with mock.patch.object(skill_loader, "MAX_SKILL_CHARS", 100):
            skill = skill_loader.load_skill()
        self.assertLessEqual(len(skill["instructions"]) + len(skill["examples"]) + len(skill["references"]), 100)
        self.assertTrue(skill["warnings"])

    def test_skill_change_invalidates_report_cache(self):
        run_research("약과", "United States", db_path=self.db)
        self.assertTrue(run_research("약과", "United States", db_path=self.db)["from_cache"])
        real = skill_loader.load_skill

        def edited(name=None):
            return {**real(name), "fingerprint": "changed"}
        with mock.patch.object(research, "load_skill", side_effect=edited):
            self.assertFalse(run_research("약과", "United States", db_path=self.db)["from_cache"])

    def test_parse_rubric(self):
        crit = skill_loader.parse_rubric("- a_b | 이름 | 설명\n- bad line\n- a_b | 중복 | x\n  - c | C | d")
        self.assertEqual([c["id"] for c in crit], ["a_b", "c"])


class V9IndependentBoothTest(V6PipelineTest):
    """v9: 부스 기획은 Tavily 조사를 쓰지 않고 입력 정보 + OpenAI 자체 조사로만."""

    def setUp(self):
        super().setUp()
        BOOTH_CALLS.clear()
        RESEARCH_CALLS.clear()
        FakeResponses.fail = False
    ...
    def run_mandu(self):
        return run_research("물만두", "United States", db_path=self.db, exhibition_name="Winter Fancy Faire 2027",
                            exhibition_website="https://www.specialtyfood.com/fancy-food-shows",
                            company_profile={"strengths": "한 입 크기, 얇고 쫄깃한 피", "ingredients": "돼지고기, 양배추, 부추, 밀가루",
                                             "certifications": "HACCP, FDA 등록", "price_range": "500g 소매가 5~7달러"})

    def booth_prompts(self):
        return [c.args[1] for c in research._ask_json.call_args_list
                if any(m in c.args[1] for m in ("스킬의 1~4단계", "부스 컨셉 기획안을 작성하세요", "채점표의 각 항목",
                                                 "채점표로 평가되었습니다"))]

    def test_booth_does_not_use_tavily_results(self):
        r = self.run_mandu()
        self.assertTrue(r["cards"])                                           # Tavily 조사는 화면용으로 그대로
        self.assertTrue(r["quotes"])
        prompts = self.booth_prompts()
        self.assertEqual(len(prompts), 4)                                     # 뼈대·초안·채점·수정
        tavily_texts = [q["quote"] for q in r["quotes"]] + ["[조사 발췌]", "[질문별 조사 요약]"]
        for prompt in prompts:
            for t in tavily_texts:
                self.assertNotIn(t, prompt)
            self.assertIn("[자체 조사 메모]", prompt)
            self.assertIn("물만두", prompt)
            self.assertIn("한 입 크기, 얇고 쫄깃한 피", prompt)
            self.assertIn("Winter Fancy Faire 2027", prompt)
        self.assertEqual(r["booth"]["source_mode"], "openai_self_research")

    def test_web_search_research_and_citation_status(self):
        r = self.run_mandu()
        call = RESEARCH_CALLS[0]
        self.assertEqual(call["model"], research.SELF_RESEARCH_MODEL)
        self.assertEqual(call["tools"][0]["type"], "web_search")
        self.assertEqual(call["tools"][0]["user_location"], {"type": "approximate", "country": "US"})
        self.assertEqual(call["instructions"], research.PLANNER_PERSONA)
        self.assertIn("HACCP, FDA 등록", call["input"])
        self.assertNotIn("Shoppers say", call["input"])
        rs = r["booth"]["research"]
        self.assertEqual(rs["method"], "web_search")
        self.assertEqual([n["id"] for n in rs["notes"]], ["R1", "R2", "R3", "R4"])       # 본문 없는 메모는 버림
        self.assertEqual([n["source_status"] for n in rs["notes"]], ["웹 출처", "출처 미확인", "AI 지식", "AI 지식"])
        self.assertEqual(rs["notes"][3]["topic"], "trend")
        self.assertEqual(rs["citations"], [{"url": "https://www.costco.com/bibigo", "title": "Costco Bibigo"}])

    def test_web_search_failure_falls_back_to_knowledge(self):
        FakeResponses.fail = True
        r = self.run_mandu()
        rs = r["booth"]["research"]
        self.assertEqual(rs["method"], "knowledge")
        self.assertIn("웹 검색 실패", rs["error"])
        self.assertEqual(rs["notes"][0]["source_status"], "AI 지식")
        self.assertEqual(rs["notes"][0]["source_url"], "")                              # 지식 모드 URL은 표시 안 함
        self.assertTrue(r["booth"]["summary"])

    def test_web_search_disabled(self):
        with mock.patch.object(research, "SELF_RESEARCH_WEB", False):
            r = self.run_mandu()
        self.assertEqual(RESEARCH_CALLS, [])
        self.assertEqual(r["booth"]["research"]["method"], "knowledge")

    def test_booth_failure_keeps_tavily_results(self):
        with mock.patch.object(research, "plan_booth_independent", side_effect=RuntimeError("openai down")):
            r = self.run_mandu()
        self.assertIsNone(r["booth"])
        self.assertIn("openai down", r["booth_error"])
        self.assertTrue(r["cards"])

    def test_flask_render_v9(self):
        import app as web
        with mock.patch.object(research, "DEFAULT_DB_PATH", self.db):
            html = web.app.test_client().post("/", data={"product_name": "물만두", "country": "United States",
                                                         "strengths": "한 입 크기"}).get_data(as_text=True)
        for text in ["v9 · Tavily 조사 + OpenAI 자체 조사 부스 컨셉", "OpenAI 자체 조사 기반", "Tavily 조사 결과를 쓰지 않고",
                     "OpenAI 자체 조사 메모 4건 · 웹 검색", "Bibigo mandu is sold at Costco", "출처 미확인",
                     'href="#R2"', "R번호 = OpenAI 자체 조사 메모"]:
            self.assertIn(text, html, text)
        self.assertNotIn("자체에 대한 현지 자료 없이", html)

    def test_extract_json(self):
        self.assertEqual(research._extract_json('앞말 ```json\n{"a": 1}\n``` 뒷말'), {"a": 1})
        self.assertEqual(research._extract_json('설명 {"a": {"b": 2}} 끝'), {"a": {"b": 2}})
        with self.assertRaises(ValueError):
            research._extract_json("JSON 없음")
