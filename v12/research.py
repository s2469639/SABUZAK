...
VERSION = "v12.0"              # 질문·프롬프트·규칙을 바꾸면 올려서 리포트 캐시 무효화
...
BOOTH_SCHEMA = """{
  ...
  "visitor_flow": {"3s": "통로에서 3초: 무엇이 눈을 멈추게 하나", "30s": "30초: 시식·시연", "3min": "3분: 상담 테이블"},
  "kpis": ["부스 성과 지표"],
...
def normalize_booth(data, quote_index, profile):
    ...
    return {
        ...
        "sections": sections,
        "visitor_flow": {k: text(data.get("visitor_flow") if isinstance(data.get("visitor_flow"), dict) else {}, k)
                         for k in ("3s", "30s", "3min")},
