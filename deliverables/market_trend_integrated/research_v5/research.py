...
원칙: AI는 사실을 쓰지 않는다. 원문에서 찾아 붙이고, 정리만 한다.
      AI의 판단(국가·제품 관련성 등)은 코드 신호로 한 번 더 확인한다.
...
VERSION = "v5.1-integrated"               # 질문·프롬프트·규칙을 바꾸면 올려서 리포트 캐시 무효화
MODEL = "gpt-4o-mini"
CACHE_TTL_DAYS = 1
...
def _search(conn, tavily, query, params, ttl):
    ...
    try:
        resp = tavily.search(query=query, include_raw_content=True, timeout=45, **params)
    except TypeError:  # 구버전 SDK
        resp = tavily.search(query=query, timeout=45, **params)
    results = []
...
JSON: {{"product_terms": [], "category_terms": [], "kfood_terms": [], "country_names": [],
    ...
    info = COUNTRIES.get(COUNTRY_ALIASES.get(country, country).lower())
    if info:
        tld, region_key = info
...
def build_queries(terms, product_name, country, exhibition_name, exhibition_website):
    ...
    if exhibition_name:
        if domain:
            add("Q4", f"{exhibition_name} {terms['category_en']} trends", {**ex, "include_domains": [domain]})
...
# 3. 검색 (K11·K12)
# ---------------------------------------------------------------
def collect_sources(conn, tavily, queries, ttl, diag):
    seen, sources, failed, excluded = set(), [], [], []
    for q in queries:
        ...
        for r in results:
            ...
            sources.append({**r, "id": len(sources), "found_by": q["qid"], "domain": domain,
...
def _check(text, ids, quote_index, terms, for_conclusion=False):
    ...
    if for_conclusion and all(q["is_stale"] or q.get("date_unverified") for q in cited):
...
def summarize_question(client, product_name, country, qid, quotes, quote_index, terms, diag):
    ...
    rows = [{"id": x["id"], "scope": x["scope_label"], "market": x["market_label"],
             "year": x["year"] or "미상", "old": x["is_stale"] or x.get("date_unverified"),
...
JSON: {{"conclusion": "...", "conclusion_ids": ["E1"], "points": [{{"text": "...", "quote_ids": ["E1"]}}],
    ...
    if ids:
        ...
        fresh = [p for p in card["points"] if not all((quote_index[i]["is_stale"] or quote_index[i].get("date_unverified")) for i in p["quote_ids"])]
        if fresh:
    ...
    with _locks[key]:
        ...
        try:
            ...
            try:
                ...
                queries = build_queries(terms, product_name, country, exhibition_name, exhibition_website)
                sources, failed, excluded = collect_sources(conn, tavily, queries, ttl, diag)
            ...
            result = {
                ...
                "sources": [{k: s.get(k) for k in ("id", "title", "url", "domain", "published_date", "year",
                                                  "market", "is_public_kr", "found_by")} for s in sources],
