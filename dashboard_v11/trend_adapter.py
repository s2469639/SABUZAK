...
def _retail():
    ...
    if _retail_module is None:
        ...
        retail_research.openai_client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"), default_headers=SAFE_HEADERS)
...
def run_section2(inputs: dict) -> dict:
    ...
    data = dict(data)
    data.pop("booth_solution", None)   # 부스 컨셉은 섹션 4(Claude 마케팅 에이전트)만 사용
    return data
