# 사부작(SABUZAK) | 한국 식품 수출기업 해외 박람회 운영 자동화 대시보드

한국 식품 수출기업의 해외 박람회 운영 업무를 자동화하는 대시보드입니다. 마이페이지에서 자신의 제품(브랜드명·HS코드·원재료·인증·가격 등)을 등록하면, 해외 시장 조사·관세율/규제 조회·부스 컨셉 기획·기안서 작성까지 하나의 화면에서 처리합니다.

특정 제품군(과자·스낵 등)에 고정된 서비스가 아니라, 등록한 제품의 HS코드를 기준으로 관세율·수출 규제·시장 트렌드를 조회하는 범용 식품 수출 지원 플랫폼입니다.
